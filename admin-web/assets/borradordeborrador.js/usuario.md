...
//LOGICA PARA MOSTRAR Y OCULTAR CAMPOS DE ABOGADOS 
function toggleAbogadoFields() {
    ...
    const rolCodigo = selectedOption ? selectedOption.dataset.codigo : '';

    if (rolCodigo === 'abogado') {
        abogadoFields.style.display = 'block';
    } else {
        abogadoFields.style.display = 'none';
        // También reseteamos los campos internos para evitar enviar datos basura
        document.getElementById('estudio-tab-pane').querySelectorAll('input').forEach(i => i.value = '');
        document.getElementById('availability-list').innerHTML = '';
        availabilityCounter = 0;
    }
}

// LÓGICA PARA AÑADIR/ELIMINAR HORARIOS DE DISPONIBILIDAD
